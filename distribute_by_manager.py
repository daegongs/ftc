...
def load_manager_info(file_path):
    ...
    try:
        ...
        df = pd.read_excel(file_path, sheet_name=sheet_name, dtype=str) # 모든 데이터를 문자로 로드
...
def copy_sheet_styles_full(source_wb, source_sheet_name, target_wb, target_sheet_name, header_row):
    """
...
def main():
    ...
    try:
        ...
        stock_df, company_col, biz_num_col, header_row, src_sheet = load_stock_holding_data(stock_file)
        ...
        for manager, df in manager_data.items():
            if len(df) > 0:
                ...
                if sheet_name in target_wb.sheetnames:
                    copy_sheet_styles_full(source_wb, src_sheet, target_wb, sheet_name, header_row)
