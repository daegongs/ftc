  ...
  for manager, companies in manager_companies.items():
    ...
    if company_col:
      def match_company(x):
        ...
        # 정확 일치만 허용 (부분 일치는 제외하여 오매칭 방지)
        # 예: "네이버"와 "네이버클라우드"를 구분
        if normalized_x in company_names:
          return True
        
        # 추가: 공백이나 특수문자 차이만 있는 경우 처리
        # 예: "네이버 클라우드" vs "네이버클라우드"
    ...
    print(f"    {manager}: {len(filtered_df)}개 행")
...
def main():
  ...
  output_file = output_dir / f'임원지분현황_담당자별배포_{datetime.now().strftime("%Y%m%d_%H%M%S")}.xlsx'
